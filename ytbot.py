...
def setup_hf_model():
    ...
    model_id = "meta-llama/Llama-3.1-8B-Instruct"   # you can replace this with any HF model
    
    # Create the Hugging Face inference endpoint LLM
    llm = HuggingFaceEndpoint(
        repo_id=model_id,
        task="text-generation",   # use "conversational" if model doesn’t support text-generation
        max_new_tokens=900,       # similar to your WatsonX parameters
        temperature=0.7,
        huggingfacehub_api_token=hf_token           # you can tune this
    )
    
    # Optionally wrap it for chat-style interaction
...
with gr.Blocks(title="🎥 AI-Powered YouTube Summarizer & QA Tool") as interface:
    gr.Markdown("## 📺 AI-Powered YouTube Summarizer & QA Tool with RAG, LangChain, FAISS")
